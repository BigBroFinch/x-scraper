...
test('getUserByScreenName', async () => {
  ...
  const response = await client.user.getUserByScreenName({ screenName: 'elonmusk' });
...
test('getUserByScreenNameNotFound', async () => {
  logger.log('getUserByScreenName');
  const client = await getClient();
  const response = client.user.getUserByScreenName({ screenName: 'Wendoleecs' });
...
test('getUserByScreenNameFreezed', async () => {
  logger.log('getUserByScreenName');
  const client = await getClient();
  const response = await client.user.getUserByScreenName({ screenName: 'anal' });
  printUser(response.data);
  expect(response.raw.response.ok).toBe(true);
});
...
test('getUserByRestId', async () => {
  ...
  const client = await getClient();
  const response = await client.user.getUserByRestId({ userId: '44196397' });
  printUser(response.data);
  expect(response.raw.response.ok).toBe(true);
});
